...
def index():
    ...
    # use Flask's request.args object to get page from query string
    page = request.args.get('page', 1, type=int)
    # use paginate() to retrieve only the desired page of results
    posts = current_user.followed_posts().paginate(
        page, current_app.config['POSTS_PER_PAGE'], False)
    # Add links to bottom of page if there is a next or prev paginate item
    next_url = url_for('main.index', page=posts.next_num) if posts.has_next else None
    prev_url = url_for('main.index', page=posts.prev_num) if posts.has_prev else None
    # render index template and pass user variable in route to user variable in index.html
    # pass the page specific posts to the template with posts.items
